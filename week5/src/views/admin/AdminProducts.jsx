...
const AdminProduct = () => {
  ...
  const handleDeleteProduct = async () => {
    try {
      setLoading(true);
      const res = await axios.delete(`${API_BASE}/api/${API_PATH}/admin/product/${modalProduct.id}`);
      alert(res.data.message);
      closeModal();
      getProducts(currentPage);
      setLoading(false);
          ...
          <tbody>
            {(products.products || []).map((product) => (
              <tr key={product.id}>
                ...
                <td><span className="text-aurora fw-bold">${product.origin_price}</span></td>
                <td><span className="text-aurora fw-bold">${product.price}</span></td>
                <td className="text-center">
                  <span className={`badge ${product.is_enabled === 0 ? 'text-secondary border-secondary' : 'text-success border-success'} bg-opacity-10 text-opacity-100 border border-current`}>
                    {product.is_enabled === 0 ? '未上架' : '已上架'}
                  </span>
                </td>
                <td className="text-end">
                  <div className="d-flex justify-content-end gap-2">
                    <button className="btn btn-sm btn-aurora-outline border-opacity-10" onClick={() => openModal(product, "edit")}>編輯</button>
                    <button className="btn btn-sm btn-outline-danger border-opacity-10" onClick={() => openModal(product, "delete")}>刪除</button>
                  </div>
                </td>
              </tr>
            ))}
